...
export default function DataImages({ color }) {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get("https://ika.sarafdesign.com/images")
      .then((res) => {
        setData(res.data);
      })
      .catch((error) => {
  ...
  let handleDelete = (e, x) => {
    if (window.confirm("Apakah anda yakin ingin menghapus?")) {
      setLoading(true);
      axios
        .delete(`https://ika.sarafdesign.com/images/${e}/${x}`)
        .then((res) => {
          alert("Kehapus");
    ...
    <>
      <div className="relative flex w-1/2 flex-wrap items-stretch mb-3 text-gray-500">
        ...
        <input
          ...
          type="text"
          placeholder="Search By Nama Image"
                  ...
                  {filterData.length === 0 && (
                    <>
                      <div className="flex justify-center items-center text-center my-8">
                        <span>Nama Image tidak ditemukan</span>
                      </div>
                    </>
                  )}
                </>
...
DataImages.defaultProps = {
  color: "light",
};

DataImages.propTypes = {
